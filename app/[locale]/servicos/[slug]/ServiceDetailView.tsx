...
export function ServiceDetailView({ sub, parentService, locale }: Props) {
  ...
  // acontecer; a substituicao funcionava e o erro ficava so no console.
  const waMsg = tContact('service_inquiry', { service: localizedSub.name })
  ...
  return (
    <>
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto w-full px-6 sm:px-10 lg:px-16 pt-36 pb-16">
          ...
          {sub.recommended && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-4">
              <span className="text-[9px] font-bold tracking-[0.2em] uppercase bg-verde-luz text-verde px-3 py-1.5 rounded-full">
                {t('recommended')}
              </span>
            </motion.div>
          )}
          <motion.h1 initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.25, ease: [0.33, 1, 0.68, 1] }}
            className="text-[clamp(48px,8vw,110px)] leading-[0.9] tracking-[-0.04em] text-menta mb-6">
            {localizedSub.name}
          </motion.h1>
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
            <AnimateIn className="lg:col-span-7">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-verde-medio">{t('about_service')}</span>
              </div>
              <p className="text-[19px] text-tinta-70 leading-[1.85]">{localizedSub.description}</p>
            </AnimateIn>
            <AnimateIn delay={0.1} className="lg:col-span-5">
              <div className="bg-menta-clara rounded-2xl p-8 border border-tinta-16">
                <div className="text-[10px] font-bold tracking-[0.18em] uppercase text-tinta-50 mb-5">{t('for_whom')}</div>
                  ...
                  {localizedSub.forWhom.map((f, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="shrink-0 w-5 h-5 rounded-full bg-verde-medio/15 flex items-center justify-center mt-0.5">
                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="#2F5E4A" strokeWidth="2" strokeLinecap="round"><path d="M2 5l2 2 4-4"/></svg>
                      </span>
      ...
      {/* MIDIA — a mesma grade da disciplina, no mesmo ponto da pagina irma */}
      {temGrade && (
        ...
        <div className="max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-16">
          <AnimateIn className="mb-12">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-5 h-px bg-verde-medio" />
              <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-verde-medio">{t('whats_included')}</span>
            </div>
            <h2 className="text-[clamp(26px,3.5vw,40px)] text-verde tracking-tight">{t('you_receive')}</h2>
          </AnimateIn>
      ...
      <section className="bg-verde py-20 lg:py-28 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-16">
          <AnimateIn>
            <div className="text-[10px] font-bold tracking-[0.2em] uppercase text-verde-luz/75 mb-4">{t('expected_result')}</div>
            <div className="text-[clamp(22px,3.5vw,42px)] font-semibold text-verde-luz leading-tight mb-12 max-w-[600px]">
              {localizedSub.result}
            </div>
